...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, norm_layer=None, branch=False):
        ...
        self.layer4_0 = self._make_layer(block, 512, layers[3], stride=2) # changed from 512 to 256
        if branch:
            self.inplanes = 256 * block.expansion
            self.layer4_1 = self._make_layer(block,512, layers[3], stride=2) # changed from 512 to 256
...
class ResNet10(nn.Module):
    ...
    def __init__(self, norm_layer=None, branch=False): 
        ...
        self.layer1 = AdvBlock(1, 64) # changed from 3 to 1
        self.layer2 = AdvBlock(64, 128)
        self.layer3 = AdvBlock(128, 128) #(128,256)
        self.layer4 = AdvBlock(128, 256) #(256,512)
    ...
    def forward(self, x):
        ...
        x = self.maxpool(x)
        x = self.layer3(x)
        x_branch = self.maxpool(x)
        x = self.maxpool(self.layer4(x_branch))
        return x_branch, x

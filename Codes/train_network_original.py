...
if __name__ == '__main__':
    ...
    gflags.DEFINE_float("hloss_alpha", 0.0, "coarse hloss factor") 
    ...
    # data loading
    trainSet = EpnetworkTrain(Flags.train_path, features=features)
    ...
    if Flags.resume:
        ...
        path_checkpoint = Flags.model_path + '/model-conv4-dropout' + str(Flags.dropout_p) +'_' +str(Flags.dropout_n)+'-'+features+'feature-new-1.0+hloss_' + str(Flags.hloss_alpha) +'-' + str(batch_id) + ".pt"
    ...
    for batch_id, (nt1, nt2, label) in enumerate(trainLoader, 1):
        ...
        if batch_id > Flags.max_iter:
            break
        if Flags.cuda:
            nt1, nt2, label = Variable(nt1.cuda()), Variable(nt2.cuda()), Variable(label.cuda())
        else:
            nt1, nt2, label = Variable(nt1), Variable(nt2), Variable(label)
        optimizer.zero_grad()
        output = net.forward(nt1, nt2)
        ...
        if batch_id % Flags.save_every == 0:
            ...
            torch.save(checkpoint_dict, Flags.model_path + '/model-conv4-dropout' + str(Flags.dropout_p) +'_' +str(Flags.dropout_n)+'-'+features+'feature-new-1.0+hloss_' + str(Flags.hloss_alpha) +'-' + str(batch_id) + ".pt")
        ...
        if batch_id % Flags.test_every == 0:
            ...
            for way_id in way_list:
                ...
                evaluation(testLoader,feature_n,i,'train_conv4_dropout'+ str(Flags.dropout_p) +'_' +str(Flags.dropout_n)+'_results_1.0+hloss_'+ str(Flags.hloss_alpha) +'_'+str(way_id)+'way_' + features +'feature')
                i= i+1
